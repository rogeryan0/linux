...
static void	SkCheckWOParam(SK_AC *pAC, SK_U16*, SK_U16*);
...
#endif
static int SK_DEVINIT SkGeTestMsi(struct SK_NET_DEVICE *dev, SK_AC *pAC);
...
struct pci_dev *pdev)   /* the device that is to resume */
{
	...
	if (pAC->GIni.GIDontInitPhy == SK_TRUE) {
		 ...
		 */
		if (!pAC->SdkQH) {
			netif_wake_queue(pAC->dev[pNet->PortNr]);
		}
...
/*****************************************************************************
 *
 *      SkCheckWOParam - Check the WOM param
 *
 * Description:
 *
 * Returns:     N/A
...
static int SK_DEVINIT SkGeRequestIrq(struct SK_NET_DEVICE *dev)
{
	...
	if (!CHIP_ID_YUKON_2(pAC)) {
		...
		if ((pAC->GIni.GIPciBus == SK_PEX_BUS) && 
			(pAC->MsiIrq) && (pci_enable_msi(pAC->PciDev) == 0)) {
			Ret = SkGeTestMsi(dev, pAC);
			if (Ret) {
				/* Error */
		...
		Ret = request_irq(dev->irq, SkY2Isr, 
			(pAC->AllocFlag & SK_ALLOC_MSI) ? 0 : IRQF_SHARED,
			dev->name, dev);
#else
		Ret = request_irq(dev->irq, SkY2Isr, 
			(pAC->AllocFlag & SK_ALLOC_MSI) ? 0 : SA_SHIRQ,
			dev->name, dev);
#endif
	}
...
static int SK_DEVINIT SkGeTestMsi(struct SK_NET_DEVICE *dev, SK_AC *pAC)
{
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,30)
	Ret = request_irq(pdev->irq, SkGeTestIsr, 0, dev->name, dev);
#else
	Ret = request_irq(pdev->irq, (irq_handler_t) SkGeTestIsr, 0, dev->name, dev);
#endif
	...
	init_waitqueue_head (&pAC->msi_wait);
...
#ifdef MV_INCLUDE_SDK_SUPPORT
	if (FwInit(pAC, pAC->IoBase, SK_INIT_DATA) != 0) {
...
#ifdef MV_INCLUDE_SDK_SUPPORT
	if (FwInit(pAC, pAC->IoBase, SK_INIT_IO) != 0) {
...
struct SK_NET_DEVICE *dev)  /* the device that is to be opened */
{
	...
	if (pAC->BoardLevel != SK_INIT_RUN) {
		...
		if (pAC->FwRun && pAC->Suspended == SK_FALSE) {
			...
			if (FwInit(pAC, pAC->IoBase, SK_INIT_RUN) != 0) {
				module_put(THIS_MODULE); /* decrease usage count */
...
struct SK_NET_DEVICE *dev)  /* the device that is to be closed */
{
	...
	if (pAC->MaxPorts == 1) {
		...
		/* Stop ports */
		spin_lock(&pAC->TxPort[PortIdx][TX_PRIO_LOW].TxDesRingLock);
		if (CHIP_ID_YUKON_2(pAC)) {
		...
		spin_unlock(&pAC->TxPort[PortIdx][TX_PRIO_LOW].TxDesRingLock);
	}
	...
	if (pAC->RlmtNets == 1) {
		...
		if (!CHIP_ID_YUKON_2(pAC)) {
			...
			ClearTxRing(pAC, &pAC->TxPort[PortIdx][TX_PRIO_LOW]);
		}
		else {
		...
		if ((pMemBuf = kmalloc(FwIoctl.Len, GFP_KERNEL)) != NULL) {
			...
			if(!copy_from_user(pMemBuf, FwIoctl.pData, FwIoctl.Len)) {
				if ( FwIoctl.Command == SK_IOCTL_FW_SEND) {
					...
					if ((pAC->FwBufferLen) && (pAC->FwBufferLen)) {
#ifdef VERBOSE_ASF_FIFO_DBUG_RPINTS
...
#endif
#if defined(CONFIG_BUFFALO_PLATFORM)
			sprintf(kernevnt_msg, "lanact %d %s %s", kernevnt_speed, (flag)? "full":"half", pAC->dev[FromPort]->name);
#endif
		} else {
...
#if defined(CONFIG_BUFFALO_PLATFORM)
				sprintf(kernevnt_msg, "lanact 0 half %s", pAC->dev[FromPort]->name);
#endif
			}
		} else {
			DoPrintInterfaceChange = SK_TRUE;
...
#else
		pNet = netdev_priv(pAC->dev[Param.Para32[0]]);
#endif

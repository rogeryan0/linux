...
//--------------------------------------------------------------
void kernevnt_IOErr(const char *kdevname, const char *dir, sector_t sector, unsigned int errcnt)
{
#ifdef FUNCMSG
		printk(">%s(%s): I/O error, dev %s, sector %llu\n",
			__FUNCTION__,kdevname, dir, sector, errcnt);
#endif
	char msg[64];
	sprintf(msg,"ioerr %s %s %llu %u",kdevname, dir, sector, errcnt);
	buffalo_kernevnt_queuein(msg);
}

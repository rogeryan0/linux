...
int __init BuffaloUpsdrv_init (void)
{
	...
	proc_buffalo_ups = proc_mkdir("ups", get_proc_buffalo());
...
#if defined(BUFFALO_SUPPORT_OMRON_UPS_SHUTDOWN)
	omr_std_en = create_proc_entry("omron_shutdown_enable", 0, proc_buffalo_ups);
...
void BuffaloUpsdrv_exit(void)
{
	...
	remove_proc_entry("ups", get_proc_buffalo());
}

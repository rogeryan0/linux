...
#define SWITCH_IS_LPORT_CONNECTED(p)                                 \
            ( ((p) == SWITCH_LPORT_CPU) || ((p) == SWITCH_PORT_0) || \
			  ((p) == SWITCH_PORT_1)   || ((p) == SWITCH_PORT_2) || \
			  ((p) == SWITCH_PORT_3)   || ((p) == SWITCH_PORT_4) ) 
#endif
...
#if defined(CONFIG_BUFFALO_PLATFORM)
unsigned int        switch_enabled_lports;
#endif
...
static int mv_gtw_get_port_mask(char **p_net_config, int idx)
{
    ...
    if (syntax_err == 0) {
#if defined(CONFIG_BUFFALO_PLATFORM)
	    gtw_config.vlan_cfg[idx].lports_mask = curr_port_mask;
#else
...
#if defined(CONFIG_BUFFALO_PLATFORM)
	if( MV_BIT_CHECK(ports_mask, p) && (p != SWITCH_LPORT_CPU) ) {
#else
...
int __init mv_gtw_net_setup(int port)
{
    ...
    ETH_DBG( ETH_DBG_LOAD, ("*************************      SWITCH_PORT_CPU - 0x%x\n",SWITCH_PORT_CPU));
    SWITCH_PORT_0   = mvBoardSwitchPortGet(port, 0);
    ETH_DBG( ETH_DBG_LOAD, ("*************************      SWITCH_PORT_0 - 0x%x\n",SWITCH_PORT_0));
    SWITCH_PORT_1   = mvBoardSwitchPortGet(port, 1);
    ETH_DBG( ETH_DBG_LOAD, ("*************************      SWITCH_PORT_1 - 0x%x\n",SWITCH_PORT_1));
    SWITCH_PORT_2   = mvBoardSwitchPortGet(port, 2);
    ETH_DBG( ETH_DBG_LOAD, ("*************************      SWITCH_PORT_2 - 0x%x\n",SWITCH_PORT_2));
    SWITCH_PORT_3   = mvBoardSwitchPortGet(port, 3);
    ETH_DBG( ETH_DBG_LOAD, ("*************************      SWITCH_PORT_3 - 0x%x\n",SWITCH_PORT_3));
    SWITCH_PORT_4   = mvBoardSwitchPortGet(port, 4);
    ETH_DBG( ETH_DBG_LOAD, ("*************************      SWITCH_PORT_4 - 0x%x\n",SWITCH_PORT_4));

#if defined(CONFIG_BUFFALO_PLATFORM)
    SWITCH_LPORT_CPU = 2; /*Yotam */
#endif
    ...
    switch_enabled_lports = (1 << SWITCH_LPORT_CPU); 
    ...
    for(i=0, nc=&gtw_config.vlan_cfg[i]; i<gtw_config.vlans_num; i++, nc++) 
    {
	    ...
	    if(nc->lports_mask & (1<<SWITCH_PORT_CPU)) 
            printk("port-CPU ");
	    if(nc->lports_mask & (1<<SWITCH_PORT_0)) 
            printk("port-0 ");
	    if(nc->lports_mask & (1<<SWITCH_PORT_1)) 
            printk("port-1 ");
	    if(nc->lports_mask & (1<<SWITCH_PORT_2)) 
            printk("port-2 ");
	    if(nc->lports_mask & (1<<SWITCH_PORT_3)) 
            printk("port-3 ");
	    if(nc->lports_mask & (1<<SWITCH_PORT_4)) 
            printk("port-4 ");
	    printk("\n");

	    /* collect per-interface port_mask into a global port_mask, used for enabling the Switch ports */
	    switch_enabled_ports |= nc->lports_mask;
	    switch_enabled_lports |= nc->lports_mask;
    }
    ETH_DBG( ETH_DBG_LOAD, ("%s : switch_enabled_ports(physical) = 0x%08x\n", __FUNCTION__, switch_enabled_ports));
    ETH_DBG( ETH_DBG_LOAD, ("%s : switch_enabled_lports(logical) = 0x%08x\n", __FUNCTION__, switch_enabled_lports));
...
static int mv_switch_init(int port)
{
...
#else
    printk("Skipping port disabling processes\n");
#endif
    ...
    switch (qd_dev->deviceId) 
    {
...
#if defined(CONFIG_BUFFALO_PLATFORM)
	        if(gprtSetDoubleTag(qd_dev, SWITCH_LPORT_CPU, GT_TRUE) != GT_OK) {
#else
...
#if defined(CONFIG_BUFFALO_PLATFORM)
	        if(gprtSetEgressMode(qd_dev, SWITCH_LPORT_CPU, GT_ADD_TAG) != GT_OK) {
#else
...
#if defined(CONFIG_BUFFALO_PLATFORM)
	        if(gvlnSetPortVlanDot1qMode(qd_dev,SWITCH_LPORT_CPU,GT_SECURE) != GT_OK) {
#else
	    ...
	    case GT_88E6065:
	        /* set CPU port number */
#if defined(CONFIG_BUFFALO_PLATFORM)
            if(gsysSetCPUPort(qd_dev, SWITCH_LPORT_CPU) != GT_OK) {
#else
...
#if defined(CONFIG_BUFFALO_PLATFORM)
	        if(gprtSetHeaderMode(qd_dev, SWITCH_LPORT_CPU, GT_TRUE) != GT_OK) {
#else
    ...
    for(i=0, nc=&gtw_config.vlan_cfg[i]; i<gtw_config.vlans_num; i++,nc++) {
        ETH_DBG( ETH_DBG_LOAD, ("vlan%d configuration (nc->ports_mask = 0x%08x) \n",
                                i, nc->ports_mask));
#if defined(CONFIG_BUFFALO_PLATFORM)
        ETH_DBG( ETH_DBG_LOAD, ("vlan%d configuration (nc->lports_mask = 0x%08x) \n",
                                i, nc->lports_mask));
#endif
	    /* set port's defaul private vlan id and database number (DB per group): */
	    for(p=0; p<qd_dev->numOfPorts; p++) {
#if defined(CONFIG_BUFFALO_PLATFORM)
	        if( MV_BIT_CHECK(nc->lports_mask, p) && (p != SWITCH_LPORT_CPU) ) {
#else
...
#if defined(CONFIG_BUFFALO_PLATFORM)
        if(mv_gtw_set_port_based_vlan(nc->lports_mask & ~(1<<SWITCH_LPORT_CPU)) != 0) {
#else
...
#if defined(CONFIG_BUFFALO_PLATFORM)
        ETH_DBG(ETH_DBG_LOAD, ("set vtu with group vlan id (used in tx)\n"));
        ETH_DBG(ETH_DBG_LOAD, ("mv_gtw_set_vlan_in_vtu(0x%08x, 0x%08x)\n", nc->vlan_grp_id, nc->lports_mask | (1<<SWITCH_LPORT_CPU)));
        if(mv_gtw_set_vlan_in_vtu(nc->vlan_grp_id, nc->lports_mask | (1<<SWITCH_LPORT_CPU)) != 0) {
#else
        ...
        /* set vtu with each port private vlan id (used in rx) */
 	    for(p=0; p<qd_dev->numOfPorts; p++) {
#if defined(CONFIG_BUFFALO_PLATFORM)
                ETH_DBG(ETH_DBG_LOAD, ("set vtu with each port private vlan id (used in rx) : p = %d\n", p));
	        if(MV_BIT_CHECK(nc->lports_mask, p) && (p!=SWITCH_LPORT_CPU)) {
                    ETH_DBG(ETH_DBG_LOAD, ("mv_gtw_set_vlan_in_vtu(0x%08x, 0x%08x)\n", MV_GTW_PORT_VLAN_ID(nc->vlan_grp_id,p), nc->lports_mask & ~(1<<SWITCH_LPORT_CPU)));
#else
	        if(MV_BIT_CHECK(nc->ports_mask, p) && (p!=SWITCH_PORT_CPU)) {
#endif
                if(mv_gtw_set_vlan_in_vtu(MV_GTW_PORT_VLAN_ID(nc->vlan_grp_id,p),
#if defined(CONFIG_BUFFALO_PLATFORM)
                                          nc->lports_mask & ~(1<<SWITCH_LPORT_CPU)) != 0) {
#else
    ...
    ETH_DBG( ETH_DBG_LOAD, ("\n"));
#if defined(CONFIG_BUFFALO_PLATFORM)
    if( gvlnSetPortVlanPorts(qd_dev, SWITCH_LPORT_CPU, port_list, cnt) != GT_OK) {
#else
    ...
    for(p=0; p<qd_dev->numOfPorts; p++) {
#if defined(CONFIG_BUFFALO_PLATFORM)
	    if (MV_BIT_CHECK(switch_enabled_lports, p)) {
#else
    ...
    for(p=0; p<qd_dev->numOfPorts; p++) {
#if defined(CONFIG_BUFFALO_PLATFORM)
	    if(MV_BIT_CHECK(switch_enabled_lports, p) && (p != SWITCH_LPORT_CPU)) {
#else
    ...
    if ((qd_dev->deviceId != GT_88E6161) && (qd_dev->deviceId != GT_88E6123) && (qd_dev->deviceId != GT_88E6165)) {
    	if (switch_irq != -1) {
...
#if defined(CONFIG_BUFFALO_PLATFORM)
	    GT_DEV_EVENT gt_event = {GT_DEV_INT_PHY, 0, 0x0};
            for(p = 0;p < qd_dev->numOfPorts; p++)
            {
                if(p != SWITCH_LPORT_CPU)
                    gt_event.phyList |= (1 << p);
            }
#else
    ...
    switch (qd_dev->deviceId) {
	    ...
	    case GT_88E6108:
            ...
            for(p=0; p<qd_dev->numOfPorts; p++) {	        
#if defined(CONFIG_BUFFALO_PLATFORM)
                if( (p != SWITCH_LPORT_CPU) && (SWITCH_IS_LPORT_CONNECTED(p)) ) {
#else
	    ...
	    case GT_88E6161:
            ...
            for(p=0; p<qd_dev->numOfPorts; p++) {	        
                ...
                if( (p != SWITCH_PORT_CPU) && (SWITCH_IS_PORT_CONNECTED(p)) ) {
#endif
                        /* Configure Register 16 page 3 to 0x1111 */
                        if(gprtSetPagedPhyReg(qd_dev,p,16,3,0x1111)) { 
                            printk("gprtSetPagedPhyReg failed (port=%d)\n", p);
                        }
                        /* Configure Register 17 page 3 to 0x0045 polarity inverse */
                        if(gprtSetPagedPhyReg(qd_dev,p,17,3,0x0045)) {
                            printk("gprtSetPagedPhyReg failed (port=%d)\n", p);
                        }
	            }
            }
...
#if defined(CONFIG_BUFFALO_PLATFORM)
    mv_gtw_set_mac_addr_to_switch(dev->dev_addr,MV_GTW_VLAN_TO_GROUP(vlan_cfg->vlan_grp_id),(1<<SWITCH_LPORT_CPU),0);
#else
...
#if defined(CONFIG_BUFFALO_PLATFORM)
    mv_gtw_set_mac_addr_to_switch(dev->dev_addr,MV_GTW_VLAN_TO_GROUP(vlan_cfg->vlan_grp_id),(1<<SWITCH_LPORT_CPU),1);
#else
...
void    mv_gtw_set_multicast_list(struct net_device *dev)
{
	...
	*/
#if defined(CONFIG_BUFFALO_PLATFORM)
	mv_gtw_set_port_based_vlan(vlan_cfg->lports_mask|(1<<SWITCH_LPORT_CPU));
#else
	mv_gtw_set_port_based_vlan(vlan_cfg->ports_mask|(1<<SWITCH_PORT_CPU));
        ...
        {
#if defined(CONFIG_BUFFALO_PLATFORM)
	        if(MV_BIT_CHECK(vlan_cfg->lports_mask, i) && (i!=SWITCH_LPORT_CPU)) 
#else
...
#if defined(CONFIG_BUFFALO_PLATFORM)
                                          vlan_cfg->lports_mask | (1<<SWITCH_LPORT_CPU)) != 0) 
#else
...
#endif
#if defined(CONFIG_BUFFALO_PLATFORM)
		if(mv_gtw_set_vlan_in_vtu(MV_GTW_PORT_VLAN_ID(vlan_cfg->vlan_grp_id,i),vlan_cfg->lports_mask&(~(1<<SWITCH_LPORT_CPU))) != 0) {
#else
		...
		status = gfdbFindAtuMacEntry(qd_dev, &mac_entry, &found);
...
#if defined(CONFIG_BUFFALO_PLATFORM)
						  (1<<SWITCH_LPORT_CPU)|(vlan_cfg->lports_mask), 1);
#else
		        ...
		        else 
                {
		            mv_gtw_set_mac_addr_to_switch(curr_addr->dmi_addr,
						  MV_GTW_VLAN_TO_GROUP(vlan_cfg->vlan_grp_id),
#if defined(CONFIG_BUFFALO_PLATFORM)
						  (mac_entry.portVec | (1<<SWITCH_LPORT_CPU)), 1);
#else
...
#if defined(CONFIG_BUFFALO_PLATFORM)
					vlan_cfg->lports_mask|(1<<SWITCH_LPORT_CPU), 1);
#else
...
static void mv_gtw_update_link_status(unsigned int p, unsigned int link_up)
{
    ...
    int                 i = 0;
#if defined(CONFIG_BUFFALO_PLATFORM)
    unsigned int        prev_lports_link = 0;
#else
...
#if defined(CONFIG_BUFFALO_PLATFORM)
	        prev_lports_link = vlan_cfg->lports_link;
#else
...
#if defined(CONFIG_BUFFALO_PLATFORM)
	        if ((vlan_cfg->lports_link & vlan_cfg->lports_mask) == 0) {
#else
...
#if defined(CONFIG_BUFFALO_PLATFORM)
                            sprintf(kernevnt_msg, "lanact %d %s eth0", kernevnt_speed, (flag)?"full":"half");
#endif
                        }
                        else {
...
#if defined(CONFIG_BUFFALO_PLATFORM)
                            sprintf(kernevnt_msg, "lanact 0 half eth0");
#endif
                        }
#if defined(CONFIG_BUFFALO_PLATFORM)
                        extern void kernevnt_SwitchHubAct(const char *msg);
                        kernevnt_SwitchHubAct(kernevnt_msg);
#endif
                    }
                }
            }

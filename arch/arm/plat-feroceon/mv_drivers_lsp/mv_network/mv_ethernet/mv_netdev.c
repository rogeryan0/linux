...
#define BUFFALO_MAX_SUPPORTED_ETH      2
#endif
...
#endif
{
    ...
    {
#if defined(CONFIG_BUFFALO_PLATFORM) && defined(CONFIG_MV_GATEWAY)
        if(!priv->isGtw)
#endif
...
int egiga_buffalo_proc_string(char *buf)
{
	...
	int i = 0;
	u32 port_status;
#if defined(CONFIG_ARCH_FEROCEON_KW) || defined(CONFIG_ARCH_FEROCEON_ORION)
...
#else
	for (i = 0; i < BUFFALO_MAX_SUPPORTED_ETH; i++)
#endif
	{
		port_status = MV_REG_READ( ETH_PORT_STATUS_REG( i ) );
		len += sprintf(buf+len,"[eth%d]\n", i);
		len += sprintf(buf+len,"MAC=%02X:%02X:%02X:%02X:%02X:%02X\n",
		    mvMacAddr[i][0], mvMacAddr[i][1], mvMacAddr[i][2],
		    mvMacAddr[i][3], mvMacAddr[i][4], mvMacAddr[i][5]);
		if (port_status & ETH_LINK_UP_MASK) {
			if (port_status & ETH_GMII_SPEED_1000_MASK)
				len += sprintf(buf+len,"link=1000Mbps\n");
			else
				len += sprintf(buf+len, "link=%sMbps\n", (port_status & ETH_MII_SPEED_100_MASK)? "100":"10");
		} else
			len += sprintf(buf+len, "link=0Mbps\n");

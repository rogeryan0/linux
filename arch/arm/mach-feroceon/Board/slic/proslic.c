...
{ unsigned char i ;
	...
	for (i=98; i < 107; i++)  diag_printf( "%X.", readDirectReg(slic_dev, i));
...
diag_printf(readDirectReg(slic_dev, 68)&4?"On hook":"Off hook");
}
...
{ 
	...
	diag_printf("Performing Manual Calibration\n");
	manualCalibrate (slic_dev);
...
int initSlic(unsigned int *slicDev, unsigned int ch, int workMode, int intMode)
{
	...
	if(status == MV_OK) 
	{
		if(work_mode) {
			...
			{
				printk("error, ch %d is not in daisy chain mode\n",pSlicDev->ch);
				return 1;
			}
		}

		TRC_REC("disable slic-%d interrupts\n",pSlicDev->ch);
	...
	{
		printk("%s: error, unknown slic type\n",__FUNCTION__);
		return 1;
	}

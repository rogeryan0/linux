...
static int
BuffaloSlideSwitchReadProc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	int len = 0;
...
static int
BuffaloEthLedReadProc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	return 0;
}
...
static void BuffaloLedInit(void)
{
	...
	for (i = 0; i < BOARD_INFO(boardId)->numLedInfo; i++) {
		...
		if (bfGppOutRegBitTest(BOARD_INFO(boardId)->pLedInfo[i].gppPinNumber))
...
//		ent = create_proc_info_entry("buffalo/gpio/switch/slide_switch", 0, 0, BuffaloSlideSwitchReadProc);
	}

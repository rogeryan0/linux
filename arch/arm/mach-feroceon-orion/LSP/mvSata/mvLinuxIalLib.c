                                ...
                                MV_U16 targetsToAdd)
{
	...
	return MV_TRUE;
	/*
The problems is that when disconnecting a disk while it still used by the kernel, then some errors
will happen, and the kernel will reserve the major & minor numbers (you can think about it
as the address) of that disk, now when that disk reconnect to the same port, the kernel will allocate
different major-minor numbers for that disk, exactly as if it was new disk, so in the customer's test,
when the disk was re-connected it was identified as sdb (where initially it was sda).

Now, if the Buffalo doesn't like this behavior, then there is a simple workaround. The workaround is
simply to remove hot-plug support from the driver, so when a disk is suddenly disconnected then
reconnected the kernel will not be notified. Doing that, the kernel will not change the "disk address".
 
Be advised that connecting new disks will also not be noticed by the kernel, that is user intervention
is needed.
 
Can Buffalo answer the below questions?
1. If the test ran previously on older IDE system? (I'm more interested in Linux kernel version
    + driver being used)
 
2. If the system that was tested doesn't have real hot swap capability and thus it is similar to
    a system with our SATA driver without the hot-swap capability.
*/
#endif
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
    IAL_ADAPTER_T   *pAdapter = pSataAdapter->IALData;

...
#if defined(CONFIG_BUFFALO_PLATFORM) && defined(CONFIG_BUFFALO_USE_KERNEVNT)
...
#if defined(CONFIG_BUFFALO_USE_KERNEVNT)
...
#endif
#if defined(CONFIG_BUFFALO_USE_KERNEVNT)
		sprintf(kernevnt_msg, "lanact %d %s eth%d", kernevnt_speed, (link.duplex == MV_ETH_DUPLEX_FULL) ? "full" : "half", port);
#endif
	} else
#if defined(CONFIG_BUFFALO_USE_KERNEVNT)
	{
		sprintf(kernevnt_msg, "lanact 0 half eth%d", port);
#endif

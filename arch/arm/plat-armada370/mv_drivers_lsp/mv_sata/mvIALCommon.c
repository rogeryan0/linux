...
#ifdef CONFIG_BUFFALO_SATA_RECONNECT // BUFFALO_PLATFORM
extern int buffalo_booting;
#endif // CONFIG_BUFFALO_SATA_RECONNECT
...
#ifdef CONFIG_BUFFALO_SATA_RECONNECT // BUFFALO_PLATFORM
		if(!buffalo_booting) {
		  printk("%s BUFFALO : mvConfigChannelDMA Failed! Change state to CHANNEL_CONNECTED\n", __FUNCTION__);
		  mvSetChannelState(ialExt,channelIndex,CHANNEL_CONNECTED);
		} else
#endif // CONFIG_BUFFALO_SATA_RECONNECT
                mvStopChannel(ialExt,
                              channelIndex,
...
#ifdef CONFIG_BUFFALO_SATA_RECONNECT // BUFFALO_PLATFORM
	    if(!buffalo_booting) {
		printk("%s BUFFALO : FatalError detected. Change state to CHANNEL_CONNECTED\n", __FUNCTION__);
		mvSetChannelState(ialExt,channelIndex,CHANNEL_CONNECTED);
	    } else
#endif // CONFIG_BUFFALO_SATA_RECONNECT
            mvStopChannel(ialExt,
...
#ifdef CONFIG_BUFFALO_SATA_RECONNECT // BUFFALO_PLATFORM
		if(!buffalo_booting) {
		    printk("%s BUFFALO : SW reset Failed, timer expired. Change state to CHANNEL_CONNECTED\n", __FUNCTION__);
		    mvSetChannelState(ialExt,channelIndex,CHANNEL_CONNECTED);
		} else
#endif // CONFIG_BUFFALO_SATA_RECONNECT
                mvStopChannel(ialExt,

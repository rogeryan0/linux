...
#define MV_UART2_LSR    (*(volatile unsigned char *)(INTER_REGS_BASE + 0x12200 + 0x14))
#define MV_UART2_THR    (*(volatile unsigned char *)(INTER_REGS_BASE + 0x12200 + 0x0 ))
#define MV_UART3_LSR    (*(volatile unsigned char *)(INTER_REGS_BASE + 0x12300 + 0x14))
#define MV_UART3_THR    (*(volatile unsigned char *)(INTER_REGS_BASE + 0x12300 + 0x0 ))
...
#endif
#if defined(CONFIG_BUFFALO_UBOOT_PARAMS) // BUFFALO_PLATFORM
	/* on Armada XP SPI_CS0 is mapped to different addr on u-boot.
	   SPI_CS0_PHYS_BASE is defined at 
	   arch/arm/mach-armadaxp/include/mach/armadaxp.h
	   So use SPI_CS0_PHYS_BASE instead of tag->u.mv_uboot.env_addr */
	env_addr = SPI_CS0_PHYS_BASE + tag->u.mv_uboot.env_offset;
...
static struct plat_serial8250_port aurora_uart2_data[] = {
	{
		.mapbase	= (INTER_REGS_PHYS_BASE | MV_UART_REGS_OFFSET(2)),
		.membase	= (char *)(INTER_REGS_BASE | MV_UART_REGS_OFFSET(2)),
		.irq		= IRQ_AURORA_UART2,
#if 0
		.flags		= UPF_SKIP_TEST | UPF_BOOT_AUTOCONF,
		.iotype		= UPIO_MEM,
#else
		.flags		= UPF_FIXED_TYPE | UPF_SKIP_TEST | UPF_BOOT_AUTOCONF,
		.iotype		= UPIO_MEM,
		.private_data	= (void *) (INTER_REGS_BASE | MV_UART_REGS_OFFSET(2) | 0x7C),
		.type		= PORT_16550A,
#endif
		.regshift	= 2,
		.uartclk	= 0,
...
static struct plat_serial8250_port aurora_uart3_data[] = {
	{
		.mapbase	= (INTER_REGS_PHYS_BASE | MV_UART_REGS_OFFSET(3)),
		.membase	= (char *)(INTER_REGS_BASE | MV_UART_REGS_OFFSET(3)),
		.irq		= IRQ_AURORA_UART3,
#if 0
		.flags		= UPF_SKIP_TEST | UPF_BOOT_AUTOCONF,
		.iotype		= UPIO_MEM,
#else
		.flags		= UPF_FIXED_TYPE | UPF_SKIP_TEST | UPF_BOOT_AUTOCONF,
		.iotype		= UPIO_MEM,
		.private_data	= (void *) (INTER_REGS_BASE | MV_UART_REGS_OFFSET(3) | 0x7C),
		.type		= PORT_16550A,
#endif
		.regshift	= 2,
		.uartclk	= 0,
...
static struct i2c_board_info __initdata armadaxp_i2c_rtc = {
...
static void __init rtc_init(void)
{
	...
	/* External I2C RTC device */
	i2c_register_board_info(0, &armadaxp_i2c_rtc, 1);
#endif
	/* SoC internal RTC device */
 ...
 */
void __init axp_tag_fixup_mem32(struct tag *t, char **from, struct meminfo *meminfo)
{
...
void axp_reset(char mode, const char *cmd)
{
	printk("Reseting...\n");
	mvBoardReset();

...
static struct irqaction axp_timer_irq = {
	.name		= "axp_tick",
	.flags		= IRQF_DISABLED | IRQF_TIMER | IRQF_PERCPU | IRQF_PROBE_SHARED,
...
void __init axp_time_init(unsigned int fabric_clk)
{
	...
	axp_timer_irq.dev_id = &axp_clkevt;
 ...
 */
//void __cpuexit axp_local_timer_stop(void)
void __cpuexit local_timer_stop(struct clock_event_device *evt)
{
	...
	remove_irq(IRQ_LOCALTIMER, &axp_timer_irq);

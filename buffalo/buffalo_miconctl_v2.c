...
{
	const unsigned char wole_msg[] = {0x01,0x3e,0x01};
...
#ifdef MICONMSG
	printk("Send Wol Enabling\n");
#endif
	//Send Wol Enabling CMD
	miconCntl_SendCmd(wole_msg,sizeof(wole_msg));
}
...
#endif
#if defined(CONFIG_ARCH_ARMADA_XP)
	/* workaround code(BTS#89865) */
	const unsigned char rsq_msg[] = {0x80,0x36};
	miconCntl_SendCmd(rsq_msg,sizeof(rsq_msg));
#endif

...
#elif defined(CONFIG_ARCH_FEROCEON_MV78XX0) || defined(CONFIG_ARCH_ARMADA_XP)
#include "rtc/ext_rtc/mvDS133x.h"
  #if defined(CONFIG_BUFFALO_USE_UPS)
  #include "linux/serial_reg.h"
  #include "buffalo/BuffaloUart.h"
  #endif
#endif
...
{
	unsigned cause;
	int pinNum = mvBoardGpioPinNumGet(BOARD_GPP_MC_IRQ, 0);

	if (pinNum < 32) {
		MV_U32 bit = BIT(pinNum);
		cause = MV_REG_READ(GPP_INT_CAUSE_REG(0));
		MV_REG_WRITE(GPP_INT_CAUSE_REG(0), ~(cause & bit));
	} else {
		MV_U32 bit = BIT(pinNum - 32);
		cause = MV_REG_READ(GPP_INT_CAUSE_REG(1));
		MV_REG_WRITE(GPP_INT_CAUSE_REG(1), ~(cause & bit));
	}
}
...
#if defined(CONFIG_USE_RS5C372) && (defined(CONFIG_ARCH_FEROCEON_ORION) || defined(CONFIG_ARCH_FEROCEON_MV78XX0) || defined(CONFIG_ARCH_ARMADA_XP))
/* implement drivers/rtc/rtc-rs5c372.c */
...
BuffaloGpio_ChangePowerStatus(uint8_t ChangeType)
{
	...
	if (MagicKey) {
		...
		BufRtcSetPowerStatus(MagicKey);
	} else
		BufRtcGetPowerStatus(&MagicKey);

	return MagicKey;
}

#if (defined(CONFIG_ARCH_FEROCEON_MV78XX0) || defined(CONFIG_ARCH_ARMADA_XP)) && defined(CONFIG_BUFFALO_USE_UPS)
unsigned int
BuffaloGpio_ReadMSR_UPSPort(void)
{
	...
	ret = 0;
	if (bfGppInRegBitTest(BIT_UPS_DCD))
		ret |= UART_MSR_DCD;
	if (bfGppInRegBitTest(BIT_UPS_RI))
		ret |= UART_MSR_RI;
	if (bfGppInRegBitTest(BIT_UPS_DSR))
		ret |= UART_MSR_DSR;

	if (BuffaloMctrlMode == BUFFALO_MODEM_CTRL_MODE_GPIO) {
		if (bfGppInRegBitTest(BIT_UPS_CTS))
			ret |= UART_MSR_CTS;

		if (bfGppInRegBitTest(BIT_UPS_RXD))
...
BuffaloGpio_WriteMCR_UPSPort(unsigned int value)
{
	...
	if (value & UART_MCR_DTR)
		bfGppOutRegBitSet(BIT_UPS_DTR);
	else
		bfGppOutRegBitClr(BIT_UPS_DTR);

	if (BuffaloMctrlMode == BUFFALO_MODEM_CTRL_MODE_GPIO) {
		if(value & UART_MCR_RTS)
			bfGppOutRegBitSet(BIT_UPS_RTS);
		else
			bfGppOutRegBitClr(BIT_UPS_RTS);
...
void BuffaloRtc_UPSRecoverDisable(void){
	BufRtcSetPowerStatus(0);
}

int BuffaloRtc_UPSRecoverReadStatus(void){
	
	int8_t data;
	BufRtcGetPowerStatus(&data);

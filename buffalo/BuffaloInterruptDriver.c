...
{
	TRACE(printk(">%s, data=%lu, PSW_LONG_WAIT_COUNT=%d\n",
...
PollingINITSWStatus(unsigned long data)
{
	...
	TRACE(printk(">%s, data=%lu, INIT_WAIT_COUNT=%d\n",
...
{
	TRACE(printk(">%s, data=%lu, FUNC_WAIT_COUNT=%d\n",
...
int __init BuffaloCpuInterrupts_init(void)
{
	...
	TRACE(printk(" >>> use_slide_power = %d, BIT_PWR_SW = %d\n",
		     use_slide_power, BIT_PWR_SW));

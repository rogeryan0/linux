...
/* 2013/02/15 h.dochi add EPIPE ERROR modify for Sock to File */
#if 1
#define EPIPE_MODIFY_C
#endif
/* 2013/02/15 h.dochi add end */
...
#else
	if(ret < 0 ){
		printk(KERN_ERR "Failed receive packet from socket to IOV, ret=%d\n", ret);
	}
  else if(ret < count){
		printk(KERN_DEBUG "Receive packet is less than expected count , ret=%d\n", ret);
	}
